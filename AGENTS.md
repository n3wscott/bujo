...
- The calendar rendering logic has its own component (`pkg/tui/components/index/calendar_model.go`). Both the main index pane and external harnesses (see `testbed calendar`) instantiate this model so selection/navigation behaviour stays consistent across the app and the standalone test UI.
