...
func (it entryItem) Description() string { return it.e.ID }
...
func New(svc *app.Service) Model {
    ...
    m := Model{
        ...
        entList:       l2,
        input:         ti,
        status:        "NORMAL: h/l move panes, j/k move, o add, i edit, x complete, > move, < future, ? help",
...
func (m *Model) loadEntries() tea.Cmd {
    ...
    return func() tea.Msg {
        if col == "" { return entriesLoadedMsg{nil} }
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
    ...
    case tea.KeyMsg:
        switch m.mode {
        case modeHelp:
            if key := msg.String(); key == "q" || key == "esc" || key == "?" { m.mode = modeNormal }
        case modeInsert:
            switch msg.Type {
            case tea.KeyEnter:
        ...
        case modeCommand:
            ...
            if msg.Type == tea.KeyEsc { m.mode = modeNormal }
        case modeNormal:
            ...
            // movement
            case "j":
                if m.focus == 0 { m.colList.CursorDown() ; cmds = append(cmds, m.loadEntries()) } else { m.entList.CursorDown() }
            case "k":
                if m.focus == 0 { m.colList.CursorUp() ; cmds = append(cmds, m.loadEntries()) } else { m.entList.CursorUp() }
            case "g":
                // support gg: handled by awaitingDD-style small window; simplest just go top on single g
                if m.focus == 0 { m.colList.Select(0); cmds = append(cmds, m.loadEntries()) } else { m.entList.Select(0) }
            case "G":
                if m.focus == 0 { m.colList.Select(len(m.colList.Items())-1); cmds = append(cmds, m.loadEntries()) } else { m.entList.Select(len(m.entList.Items())-1) }
            ...
            case "x":
                if it := m.currentEntry(); it != nil {
                    if _, err := m.svc.Complete(m.ctx, it.e.ID); err != nil {
                        cmds = append(cmds, func() tea.Msg { return errMsg{err} })
                    } else { m.status = "Completed"; cmds = append(cmds, m.refreshAll()) }
                }
                ...
                if it := m.currentEntry(); it != nil {
                    if m.awaitingDD && time.Since(m.lastDTime) < 600*time.Millisecond {
                        if _, err := m.svc.Strike(m.ctx, it.e.ID); err != nil {
                            cmds = append(cmds, func() tea.Msg { return errMsg{err} })
                        } else { m.status = "Struck"; cmds = append(cmds, m.refreshAll()) }
                        m.awaitingDD = false
                    } else {
                        m.awaitingDD = true
            ...
            case "<":
                if it := m.currentEntry(); it != nil {
                    if _, err := m.svc.Move(m.ctx, it.e.ID, "Future"); err != nil {
                        cmds = append(cmds, func() tea.Msg { return errMsg{err} })
                    } else { m.status = "Moved to Future"; cmds = append(cmds, m.refreshAll()) }
                }
            ...
            case "b":
                // cycle Task -> Note -> Event
                switch m.pendingBullet {
                case glyph.Task: m.pendingBullet = glyph.Note
                case glyph.Note: m.pendingBullet = glyph.Event
                default: m.pendingBullet = glyph.Task
                }
            // set bullet on selected entry
            case "T":
                if it := m.currentEntry(); it != nil { m.applySetBullet(&cmds, it.e.ID, glyph.Task) }
            case "N":
                if it := m.currentEntry(); it != nil { m.applySetBullet(&cmds, it.e.ID, glyph.Note) }
            case "E":
                if it := m.currentEntry(); it != nil { m.applySetBullet(&cmds, it.e.ID, glyph.Event) }

            // signifiers
            case "*":
                if it := m.currentEntry(); it != nil { m.applyToggleSig(&cmds, it.e.ID, glyph.Priority) }
            case "!":
                if it := m.currentEntry(); it != nil { m.applyToggleSig(&cmds, it.e.ID, glyph.Inspiration) }
            case "?":
                if m.mode == modeNormal { m.mode = modeHelp } else { m.mode = modeNormal }
...
func (m *Model) currentEntry() *entryItem {
    if len(m.entList.Items()) == 0 { return nil }
...
func (m *Model) applySetBullet(cmds *[]tea.Cmd, id string, b glyph.Bullet) {
    if _, err := m.svc.SetBullet(m.ctx, id, b); err != nil {
        *cmds = append(*cmds, func() tea.Msg { return errMsg{err} })
    } else { m.status = "Bullet updated"; *cmds = append(*cmds, m.refreshAll()) }
}

func (m *Model) applyToggleSig(cmds *[]tea.Cmd, id string, s glyph.Signifier) {
    if _, err := m.svc.ToggleSignifier(m.ctx, id, s); err != nil {
        *cmds = append(*cmds, func() tea.Msg { return errMsg{err} })
    } else { m.status = "Signifier toggled"; *cmds = append(*cmds, m.refreshAll()) }
}
