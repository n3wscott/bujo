...
func (m model) Init() tea.Cmd {
	return tea.Batch(
		func() tea.Msg {
			return charmclient.NewClientMsg(&client.Client{})
		},
...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	// TODO: only when the menu has focus, send it message events.
	...
	case charmclient.NewClientMsg:
		...
		m.info = info.NewModel(nil)
		m.username = username.NewModel(nil)
		...
		return m, func() tea.Msg {
			now := time.Now()
			return info.GotBioMsg(&charm.User{
				ID:        123,
				CharmID:   "abc",
				PublicKey: nil,
				Name:      "Foo",
